...
import { AlertTriangle, CheckCircle, Copy, Loader2, Plus, RefreshCw, Trash, Share2, Mail, Check, ArrowLeft } from "lucide-react"
...
export default function CouponsPage() {
  ...
  const handleBulkShare = (method: string) => {
    ...
    const couponsText = selectedCoupons.map(coupon => 
      ...
      toast({
        ...
        action: (
          <div className="flex space-x-2">
            <ToastAction 
              altText="WhatsApp" 
              onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`Coupon Code: ${code}\nDiscount: 100%\nOne-time use only`)}`, '_blank')}
            >
              <Share2 className="h-4 w-4 mr-1" /> WhatsApp
            </ToastAction>
            <ToastAction 
              altText="Email" 
              onClick={() => window.open(`mailto:?subject=Your Discount Coupon&body=${encodeURIComponent(`Coupon Code: ${code}\nDiscount: 100%\nOne-time use only`)}`, '_blank')}
            >
  ...
  return (
    <div className="container mx-auto py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6 px-4 sm:px-6">
              ...
              <div className="overflow-x-auto">
                ...
                <TableBody>
                  {loading ? (
                    ...
                    coupons.map((coupon) => (
                      <TableRow key={coupon.id} className={selectedCoupons.some(c => c.id === coupon.id) ? "bg-muted/40" : ""}>
                        <TableCell>
                          <input 
                            type="checkbox" 
                            className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
      ...
      <Dialog open={showBulkShareDialog} onOpenChange={setShowBulkShareDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Share {selectedCoupons.length} {selectedCoupons.length === 1 ? 'Coupon' : 'Coupons'}</DialogTitle>
            <DialogDescription>
              Choose how you want to share the selected coupons.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="sm:justify-start">
            <Button
              type="button"
              variant="secondary"
              onClick={() => setShowBulkShareDialog(false)}
            >
              Cancel
            </Button>
          </DialogFooter>
