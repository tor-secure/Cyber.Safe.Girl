...
import { Download, Share2, Loader2, AlertTriangle, Lock, Copy, Check } from "lucide-react"
...
export function Certificate() {
        ...
        if (progressData.progress.certificateUnlocked) {
          ...
          try {
            ...
            if (!certificateResponse.ok) {
              ...
              throw new Error(errorData.error || `HTTP error! status: ${certificateResponse.status}`)
            }
            ...
            if (certificateData.certificate) {
              ...
              // Use the preview URL directly from the API response
              if (certificateData.certificatePreviewURL) {
                setCertificateUrl(certificateData.certificatePreviewURL)
                console.log("Using certificate preview URL from API response:", certificateData.certificatePreviewURL)
              } else {
                console.error("Certificate preview URL not provided in API response")
  ...
  const handleDownload = async () => {
    ...
    try {
      // Get certificate with stored encryption parameters
      const certWithParamsResponse = await fetch("/api/certificate", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!certWithParamsResponse.ok) {
        throw new Error("Failed to get certificate download URL")
      }

      const certWithParamsData = await certWithParamsResponse.json()
      const downloadUrl = certWithParamsData.certificateDownloadURL
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (!certWithParamsResponse.ok) {
        throw new Error("Failed to get certificate preview URL")
      }

      const certWithParamsData = await certWithParamsResponse.json()
      const previewUrl = certWithParamsData.certificatePreviewURL
